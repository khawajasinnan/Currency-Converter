...
function calculate() {
    ...
    fetch(`https://v6.exchangerate-api.com/v6/b88b5967d64b35931fb025f7/latest/${from_currency_value}`)
        ...
        .then(res => {
            ...
            saveToHistory(from_currency_value, to_currency_value, input_amount.value, to_amount);
...
async function updateChart(fromCurrency, toCurrency) {
    try {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            // Generate a slightly different rate for each day
            const variation = (Math.random() - 0.5) * 0.1; // ±5% variation
            rates.push((currentRate * (1 + variation)).toFixed(4));
        }
